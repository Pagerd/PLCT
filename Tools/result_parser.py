...
class TestResults(object):
    def __init__(self,workingDir):
        ...
        self.suite2casesDir = 'suite2cases_out'
        self.testResult = []
    ...
    def classifyResults(self):
        for i in range(len(self.testResult)):
            for j in range(len(self.testResult[i]['failedCases'])):
                logPath = self.workingDir+'/logs/'+self.testResult[i]['name']+'/'+self.testResult[i]['failedCases'][j]['name']+'/'+self.testResult[i]['failedCases'][j]['logname']
